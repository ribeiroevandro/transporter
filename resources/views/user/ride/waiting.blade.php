...
<script src="https://assets.pagar.me/checkout/1.1.0/checkout.js"></script>
    ...
    <script type="text/javascript">
    	function call(amount,request_id){
			  ...
			  function handleSuccess (data) {
			    console.log(data);
			     $.ajax({
			            type: "POST",
			            url: "{{url('pagar/flow')}}",
			            data:{ payment_id: data.token ,request_id: request_id,amount:amount},           
			            dataType: "json",
			            success: function(data) {
    ...
    <script type="text/jsx">
		...
		var SwitchState = React.createClass({
			...
			render: function(){
				...
				if(this.props.checkState != ""){
					...
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode != 'CASH' && this.props.checkState.payment_mode != 'PAGARME' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndPayment checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode == 'PAGARME' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndPagarme checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
					}else if(this.props.checkState.status == 'COMPLETED'){
						return (
		...
		var DroppedAndPagarme = React.createClass({
			...
			submitpagarme: function(e){   
				let update_value = this.state.payable;
            
				call(update_value,this.props.checkState.id);		    
			},
			...
			render: function(){
				...
				<div>
					...
					<div className="status">
                    	...
                    	<button type="button" id="rzp-button1" onClick={this.submitpagarme.bind(this)} data-bookingid={this.props.checkState.booking_id} className="full-primary-btn fare-btn">CONTINUE TO PAY - {this.props.currency}{this.state.payable.toFixed(fixval)}</button>   
                    </form>
		        </div>
				);
			}
