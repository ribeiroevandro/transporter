            		...
            		<div class="col-md-12">
						...
						<?php $background = asset('admin/assets/img/photos-1/4.jpg'); ?>
							...
							<div class="u-content">
								...
								<p class="text-muted">Email : @if(Setting::get('demo_mode', 0) == 1) {{ substr($provider->email, 0, 3).'****'.substr($provider->email, strpos($provider->email, "@")) }} @else {{$provider->email}} @endif</p>
								<p class="text-muted">Mobile : @if(Setting::get('demo_mode', 0) == 1) {{ substr($provider->mobile, 0, 5).'****' }} @else {{ $provider->mobile }} @endif</p>
