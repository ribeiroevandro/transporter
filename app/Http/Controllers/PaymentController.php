...
class PaymentController extends Controller
{
    ...
    public function add_money_pagar(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           $url = url('/add/pagar/?amount=').$amount.'&user_id='.$user_id;
     ...
     public function add_money_pagar_api(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           return view('user.account.walletapi',compact('amount','user_id'));
  
    }

    public function provider_add_money_pagar(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           $url = url('/add/pagar/provider/?amount=').$amount.'&user_id='.$user_id;
     ...
     public function provider_add_money_pagar_api(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           return view('provider.wallet.walletapi',compact('amount','user_id'));
  
    }

    public function flow_pagar(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           $url = url('/flow/pagar/?amount=').$amount.'&user_id='.$user_id.'&request_id='.$request_id;
           
          return response()->json(['url' => $url,'status'=>'true']);
    }

     public function flow_pagar_api(Request $request)
    { 
           $this->validate($request, [
             'amount' => 'required'
           ]);
           ...
           return view('user.account.flowapi',compact('amount','request_id','user_id'));
  
    }

    public function pagar_success(Request $request){
      
     // dd($request->all());
       if(isset($request->payment_id)){


          $url = "https://api.pagar.me/1/transactions/".$request->payment_id."/capture?amount=".$request->amount."&api_key=".env('PAGARME_API_KEY');

          $payment = pagar_transaction_status($url);
           ...
           if(@$payment->status=="paid"){
                ...
                $log->save();
                 ...
                 $user = \App\User::find($log->user_id);
                $wallet = (new TripController)->userCreditDebit($amount, $user->id, 1);
                (new SendPushNotification)->WalletMoney($user->id, currency($log->amount));
               ...
               $wallet_balance = $user->wallet_balance+$amount;
                ...
                if ($request->ajax()) {
                    return response()->json(['success' => currency($amount) . " " . trans('api.added_to_your_wallet'), 'message' => currency($amount) . " " . trans('api.added_to_your_wallet'), 'wallet_balance' => $wallet_balance]);
                } else {
    ...
    public function provider_pagar_success(Request $request){
       ...
       if(isset($request->payment_id)){
          ...
          $payment = pagar_transaction_status($url);
           ...
           if(@$payment->status=="paid"){
                ...
                $wallet = (new TripController)->providerCreditDebit($amount, $user->id, 1);
                (new SendPushNotification)->ProviderWalletMoney($user->id, currency($log->amount));
    ...
    public function pagar_flow(Request $request){
          ...
          $payment = pagar_transaction_status($url);
           ...
           if(@$payment->status=="paid"){
                ...
                $RequestPayment = UserRequestPayment::where('request_id',$request->request_id)->first(); 
                  ...
                  $RequestPayment->payment_mode = 'PAGARME';
                  $RequestPayment->card = $RequestPayment->payable;
                  $RequestPayment->payable = 0;
                  $RequestPayment->tips = $tip_amount;
                  $RequestPayment->provider_pay = $RequestPayment->provider_pay+$tip_amount;
