...
{	
	...
	public function send_request(Request $request) {
		...
		$this->validate($request, [
				...
				'use_wallet' => 'numeric',
				'payment_mode' => 'required|in:BRAINTREE,CASH,CARD,PAYPAL,PAYPAL-ADAPTIVE,PAYUMONEY,PAYTM,PAGARME',
	...
	public function modifiy_request(Request $request) {
		...
		$this->validate($request, [
				...
				'payment_mode' => 'sometimes|nullable|in:BRAINTREE,CASH,CARD,PAYPAL,PAYPAL-ADAPTIVE,PAYUMONEY,PAYTM,PAGARME',
				'card_id' => ['required_if:payment_mode,CARD','exists:cards,card_id,user_id,'.Auth::user()->id],
			]);
    ...
    public function send_otp(Request $request){
        ...
        try{
            ...
            if(count($user)){
                ...
                $send_sms = sendSMS($mobile,$otp);
		              
                if(@$send_sms->status == "Sucesso"){

                    return response()->json(['status'=> 'success', 'otp' => $otp]);
                }else{

                    return response()->json(['status'=> 'failure', 'message'=> @$send_sms->erro], 422);
                }
